...
parentQueue::parentQueue() //конструктор - инициализируем размер и последний элементы
{
	size = 0;
	last = nullptr;
}

parentQueue::~parentQueue() //деструктор
{
...
void parentQueue::copy(parentQueue& Q) //метод копирования очереди
{
	...
	delete[] buff;
	//cout<< "Копированиезавершено!" << endl;
}
